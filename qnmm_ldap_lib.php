...
function qnmm_ldap_bind($username,$password)
{
  $b = false;
  global $error;
...
function qnmm_ldap_search($username)
{
  global $error;
  $error = '';
  $mail = '';
...
function qnmm_ldap_profile($username,$password,$mail='')
{
  if ( empty($mail) ) $mail = qnmm_ldap_search($username); // seach the user mail from ldap (may be empty)
  return cVIP::AddUser($username,$password,$mail);
}