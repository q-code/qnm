...
include 'qnmm_ldap_lib.php';

// INITIALISE

$tt=0;
$oVIP->selfurl = 'qnmm_ldap_adm.php';
$oVIP->selfname = 'LDAP stettings';
$oVIP->exiturl = 'qnm_adm_secu.php';
...
if ( isset($_POST['ok']) && $tt==0 )
{
  ...
  // Enable module (m_ldap=0 means disable, m_ldap=qnmm_ldap_login.php means enabled
...
if ( isset($_POST['ok']) && $tt>0 )
{
  ...
  if ( $tt==2 )
  {
    ...
    {
      $step++;
      $b = qnmm_ldap_bind($username,$password);
      if ( $b )
      {
...
echo '<div class="pan">
<div class="pan_top">',$arrTabs[$tt],'</div>
';
...
{
echo '<p class="small">Current authority is <span class="bold italic">Internal authority (default)</span>.<br/>When module is on-line, change the authority from the page '.L('Adm_security').'.</p>';
}
echo '<form method="post" action="',Href(),'">
<table class="data_o">
<tr class="data_o"><td class="headgroup" colspan="3">Module status</td></tr>
<tr class="data_o">
...
<tr class="data_o"><td class="headgroup" colspan="3">User authentication</td></tr>
<tr class="data_o">
<td class="headfirst"><label for="newuser">Login users</label></td>
<td colspan="2">
<input type="radio" id="log_all" name="m_ldap_users" value="all"',($_SESSION[QT]['m_ldap_users']=='all' ? QCHE : ''),'/><label for="log_all">Accept locally registered users AND ldap users</label><br/>
<input type="radio" id="log_ldap" name="m_ldap_users" value="ldap"',($_SESSION[QT]['m_ldap_users']=='ldap' ? QCHE : ''),'/><label for="log_ldap">Accept ONLY valid ldap users</label>
</td>
</tr>
';
...
<td colspan="2">
<p class="small"><span class="bold italic">Accept locally registered users AND ldap account</span><br/>
...
<p class="small"><span class="bold italic">Accept ONLY valid ldap accounts</span><br/>
...
With this option, it\'s recommanded to turn the registration mode to "backoffice" (see security page).</p>
</td>
</tr>
';
}
...
echo '<form method="post" action="',Href(),'">
<table class="data_o">
<tr class="data_o"><td class="headgroup" colspan="3">Connection and authentication</td></tr>
';
echo '<tr class="data_o">
...
';
echo '<tr class="data_o"><td class="headgroup" colspan="3">Search configuration (to create new user)</td></tr>
<tr class="data_o">
...
echo '<tr class="data_o"><td class="headgroup" colspan="3">Test</td></tr>
';
...
{
echo '<tr class="data_o">
<td class="headgroup" colspan="3" style="padding:6px; text-align:center"><input type="hidden" name="tt" value="',$tt,'"/><input type="submit" name="ok" value="',$L['Save'],'"/></td>
</tr>
';
}
