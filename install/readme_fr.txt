...
Pour passer de la version 1beta à 1.0, vous pouvez procéder à une installation standard (voir ci-après).
...
- Le type de base de donnée que vous utilisez (MySQL, SQLserver, PostgreSQL, SQLite, Firebird, Oracle, MS-Access (ODBC) ou DB2).
- Le nom de l'hote de votre base de donnée (le nom du serveur de base de donnée, souvent "localhost").
- Le nom de votre base de donnée (où QNM peut installer ses tables).
...
---------------------------------------------
Vous devez simplement envoyer (ftp) tous les fichiers et repertoires sur votre espace web (par exemple dans un répertoire /qnm/).
